...
1.  **修复 Kernel 运行路径问题**: 解决了通过插件打开 Notebook 时，`pwd` 默认在用户根目录的问题。现在 Kernel 会自动切换到当前 Notebook 所在的远程目录，并确保 `sys.path` 正确包含当前路径。
2.  **增强终端功能**: 在远程文件浏览器中右键打开终端时，会自动定位到当前选中的目录（CWD）。同时改进了终端异常断开时的诊断提示。
3.  **对齐 Notebook 执行体验**: 修复了 Cell 执行结果不断追加的问题，确保单次执行结果不与旧结果重叠，并支持 `IPython.display.clear_output`。
4.  **路径安全性与健壮性**: 针对远程文件路径中的特殊字符（如单引号）进行了转义处理，防止 Python 注入逻辑出现语法错误。
5.  **版本标识**: 更新了版本号及作者信息，以便区分官方版本并提供长期维护支持。
